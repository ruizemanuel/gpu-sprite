...
- Do not lower a gate. The Brotli budget is 40,000 bytes (`packages/core/scripts/size.ts`). The fixed quality thresholds (uniqueness, median Hamming distance) live in `packages/training/metrics.py`. The data-derived thresholds (coherence, density) are computed by `data.py` into `packages/training/data/stats.json`; change them only by changing the data, never by editing the file. Reconstruction floors are remembered per dataset in `active/report.json` (`floors_by_dataset`, keyed by the sha256 of the training and validation arrays, carried forward by every export), and a recorded floor never goes down. A dataset without a recorded floor starts its own, and the gate output labels it `new-dataset` (`first-promotion` when no floor exists at all): treat that label as a reset to explain, not a routine result. Any byte-level change to the arrays is a new dataset. The whole-pack dataset's floor predates the map and is not in it.
...
- The weight payload is a contract shared by `quantize.py` and `decode.ts`: int6 per-tensor weights, one character per weight from `WEIGHT_ALPHABET` (character `i` is `q = i - 31`), biases rounded to 3 decimals, `format: 2`. Change both or neither, bump `format`, then re-export.
