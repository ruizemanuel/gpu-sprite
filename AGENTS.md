...
- The seed → latent algorithm is a contract shared by `seed.ts` and `seed.py`; changing it requires bumping `SEED_VERSION` in both, regenerating `seed-vectors.json`, and re-exporting.
